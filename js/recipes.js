...
function _compileOutput(stock, type) {
  let output = [];
  stock.forEach((stockElement) => {
    if (output.includes(new Tag({
      name: stockElement.toLowerCase(),
      type
    }))) return;
    output.push(new Tag({
      name: stockElement.toLowerCase(),
  ...
  output = output.sort((a, b) => (a === b ? 0 : (a > b ? 1 : -1)));
...
function getIngredients() {
  ...
  return _compileOutput(stock, 'ingredient');
}
...
function getUstensils() {
  ...
  getRecipes().forEach((recipe) => {
    ...
    ustensils.forEach((ustensils) => {
      stock.push(ustensils);
    });
  });

  return _compileOutput(stock, 'ustensil')
}
